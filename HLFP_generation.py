"""
Adapted from https://github.com/HamidHadipour/Deep-clustering-of-small-molecules-at-large-scale-via-variational-autoencoder-embedding-and-K-means
"""
...
def atom_features_onehot(atom): # size: 151
	features = one_hot_encoding(atom.GetAtomicNum() - 1, ATOM_FEATURES['atomic_num']) + \
			   ...
			   [0] # [atom.GetMass() * 0.01]  # scaled to about the same range as other features
	return features
...
def mol2local(mol, onehot=False, pca=False, ids=None):
	# if ids is not None:
	...
	return LocalFeatures(mol, onehot, pca, ids)

# if __name__ == '__main__':
	# data = pd.read_csv("comp-ann-test.csv")
	...
	# f_atoms_pca.to_csv('atom_features_PCA.csv', index=False, sep=';')
	# f_bonds_pca.to_csv('bond_features_PCA.csv', index=False, sep=';')
	# print('Done!')
